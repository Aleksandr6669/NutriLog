...
''';
    ...
    for (final ingredient in ingredients) {
      ...
      if (searchResults.isNotEmpty) {
        ...
        if (product != null && product['foodNutrients'] != null) {
          ...
          facts.add(
              '"${ingredient.name}" (на 100 г): ${nutrientKeys.map((k) => '$k: {_usdaNutrientValue(k, nutrMap)}').join(', ')}');
        }
      }
    }
  ...
  Оцени пищевую ценность рецепта на основе списка и грамовки/штук ингредиентов.
  ...
  Твои данные по ингредиентам:${(facts.isNotEmpty ? '\n\nДанные из USDA FoodData Central по ингредиентам:\n${facts.join('\n')}' : '').isEmpty ? ' (нет данных)' : facts.isNotEmpty ? '\n\nДанные из USDA FoodData Central по ингредиентам:\n${facts.join('\n')}' : ''}
  ${facts.isNotEmpty ? '\n\nДанные из USDA FoodData Central по ингредиентам:\n${facts.join('\n')}' : ''}
  Рецепт:
